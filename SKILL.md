...
6. 每个论文部分写入 `论文草稿.docx` 后，先提交用户审核；用户确认通过后才冻结该部分并进入下一部分。用户要求修改时，修改后重新提交审核。
...
| 绘图与配色 | `references/绘图参考/chart_knowledge_base.md`（60+ 图表速查/图级/避坑）→ `references/绘图参考/chart_selection.md`（论证意图）→ `scripts/plot_style.py`、`assets/期刊配色方案.md`、`references/写作与图表规范.md` |
